...
import { Plus, Edit, Trash2, Music, BookOpen, Calendar, X, Search, Filter, SortAsc, SortDesc, Grid, List, Eye, EyeOff } from "lucide-react"
...
export default function JournalsPage() {
  ...
  const [sortBy, setSortBy] = useState<'date' | 'title' | 'author' | 'song'>('date')
  ...
  const filteredItems = useMemo(() => {
    ...
    return filtered
  }, [items, searchQuery, sortBy, sortOrder, dateFilter, startDate, endDate, authorFilter])
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const authorIds = [...new Set(data?.map(j => j.author).filter(Boolean) || [])]
        ...
        const list = ((data || []) as any[]).map(item => ({
          ...item,
          author_name: item.author ? (authorMap.get(item.author) || `User ${item.author.slice(0, 8)}`) : null
        })).filter(j => (j.title && j.title.trim()) || (j.content && j.content.trim())) as Journal[]
        setItems(list)
      } catch (e: any) {
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20 sm:pb-12">
        ...
        <div className="mb-8 space-y-4">
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                <Select value={sortBy} onValueChange={(value: 'date' | 'title' | 'author' | 'song') => { setSortBy(value); setPage(1) }}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => { setDateFilter(option.value as any); setPage(1) }}
                        className="text-xs"
                      >
                        {option.label}
                      </Button>
          ...
          {/* Results Summary */}
          {(searchQuery || dateFilter !== 'all' || authorFilter !== 'all' || startDate || endDate) && (
            <p className="text-sm text-gray-600">
              Found {filteredItems.length} note{filteredItems.length !== 1 ? 's' : ''}
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-500 mb-6">
              {searchQuery ? `No notes match "${searchQuery}"` : 'Start capturing your thoughts with music'}
            </p>
      ...
      <Dialog open={dialogOpen} onOpenChange={closeDialog}>
       ...
       <Dialog open={!!viewOpen} onOpenChange={() => setViewOpen(null)}>
         <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-hidden p-0 bg-gradient-to-b from-amber-50/30 to-white [&>button]:hidden">
               ...
               <div className="absolute bottom-4 left-6 right-6">
                 <div className="flex items-center gap-4">
                   <img 
                     src={viewOpen.spotify_image} 
                     alt={viewOpen.spotify_track_name || "Song"}
                     className="w-16 h-16 rounded-lg object-cover shadow-lg border-2 border-white/50" 
                   />
                   <div>
                     <div className="font-handwriting text-xl text-white drop-shadow-lg">
                       {viewOpen.spotify_track_name}
                      </div>
                     <div className="font-handwriting text-sm text-white/90 drop-shadow">
                       by {viewOpen.spotify_artists}
                  </div>
              </div>
            </div>
          </div>
           ...
           {/* Action buttons on the right side below song theme */}
           <div className="flex justify-end p-4 bg-white/90 backdrop-blur-sm border-b border-gray-200">
          ...
          >
            ...
            <div className="relative mb-8 pl-24">
              ...
              <div className="mt-4 flex items-center gap-4 font-handwriting text-lg text-gray-500">
                <span>{viewOpen?.created_at ? new Date(viewOpen.created_at).toLocaleDateString() : ''}</span>
                <span>•</span>
